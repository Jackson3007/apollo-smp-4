...
public class WorthTags implements Listener {
    ...
    public boolean enabled() {
        ...
        if (!plugin.getConfig().getBoolean("sell.worth-lore", true)
                && !plugin.getConfig().isSet("sell.worth-display")) {
            return false;
        }
    ...
    public void showActionBar(Player player, ItemStack stack) {
        ...
        if (!plugin.sell().isSellable(stack)) return;
        double total = plugin.sell().valueOf(stack);
        if (total <= 0) return;
        int amount = Math.max(1, stack.getAmount());
        double unit = total / amount;
        String text = amount > 1
                ? "<gray>" + com.apollosmp.util.Items.pretty(stack.getType())
                        + " <dark_gray>|</dark_gray> <#f9d423>" + plugin.msg().money(unit)
                        + "</#f9d423> <gray>each <dark_gray>|</dark_gray> <gray>stack <#f9d423>"
                        + plugin.msg().money(total) + "</#f9d423>"
                : "<gray>" + com.apollosmp.util.Items.pretty(stack.getType())
                        + " <dark_gray>|</dark_gray> <gray>sells for <#f9d423>"
                        + plugin.msg().money(total) + "</#f9d423>";
        player.sendActionBar(Msg.mm(text));
    }

    /** Refresh the action bar when a player switches which item they're holding. */
    @EventHandler
