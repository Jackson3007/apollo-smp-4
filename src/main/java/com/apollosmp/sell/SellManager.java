...
public class SellManager {
    ...
    private void loadDefaults() {
        ...
        put("SUGAR_CANE", 0.5); put("SUGAR", 0.5);
        put("PUMPKIN", 1); put("CARVED_PUMPKIN", 1); put("PUMPKIN_SEEDS", 0.2);
        ...
        // ---- Mob drops ----
        put("ROTTEN_FLESH", 0.25); put("BONE", 1); put("BONE_MEAL", 0.3); put("BONE_BLOCK", 9);
        put("STRING", 1); put("GUNPOWDER", 2); put("SPIDER_EYE", 1.5); put("FERMENTED_SPIDER_EYE", 2);
        // Ender eye must stay <= pearl + blaze powder (5 + 3) or you could craft & flip for profit.
        put("ENDER_PEARL", 5); put("ENDER_EYE", 8);
        put("BLAZE_ROD", 6); put("BLAZE_POWDER", 3);
        ...
        put("NETHER_STAR", 500); put("WITHER_SKELETON_SKULL", 40);
        put("ECHO_SHARD", 15); put("SCULK", 0.5); put("SCULK_VEIN", 0.5); put("SCULK_CATALYST", 5);
        put("TOTEM_OF_UNDYING", 30); put("EXPERIENCE_BOTTLE", 5);
        ...
        put("SUGAR_CANE", 0.5); put("TWISTING_VINES", 0.3); put("WEEPING_VINES", 0.3);
    ...
    /**
     * Sell-price multiplier for a player. Apollo+ members earn a configurable
     * bonus on everything they sell. Everyone else is 1.0 (no change).
     */
    public double sellMultiplier(Player player) {
        if (player == null || !player.hasPermission("apollo.plus")) return 1.0;
        double pct = plugin.getConfig().getDouble("ranks.apollo-plus.sell-bonus-percent", 10.0);
    ...
    public Result sellHand(Player player) {
        ...
        // valueOf accounts for gear, enchantments and wear (and the whole stack),
        // so selling by hand pays the same as the /sell menu instead of the base price.
        double earned = valueOf(hand) * sellMultiplier(player);
    ...
    public Result sellAll(Player player) {
        double mult = sellMultiplier(player);
        int totalQty = 0;
